...
+ (void)load
{
	...
	dispatch_once(&onceToken, ^{
		...
		{
			LNSwizzleMethod(cls, NSSelectorFromString(@"greyswizzled_resume"), @selector(__dtx_resume), &error);
		}
		else
		{
			LNSwizzleMethod(cls, @selector(resume), @selector(__dtx_resume), &error);
		}
		
		LNSwizzleMethod(cls, @selector(connection:didReceiveResponse:completion:), @selector(__dtx_connection:didReceiveResponse:completion:), &error);
		LNSwizzleMethod(cls, @selector(connection:didFinishLoadingWithError:), @selector(__dtx_connection:didFinishLoadingWithError:), &error);
		LNSwizzleMethod(cls, @selector(connection:didReceiveData:completion:), @selector(__dtx_connection:didReceiveData:completion:), &error);
		LNSwizzleMethod(cls, @selector(connection:didFinishCollectingMetrics:completion:), @selector(__dtx_connection:didFinishCollectingMetrics:completion:), &error);
		if(NO == LNSwizzleMethod(cls, @selector(initWithOriginalRequest:ident:taskGroup:), @selector(initWithOriginalRequest__dtx:ident:taskGroup:), &error))
		{
			LNSwizzleMethod(cls, @selector(initWithOriginalRequest:updatedRequest:ident:session:), @selector(initWithOriginalRequest__dtx:updatedRequest:ident:session:), &error);
		}
		LNSwizzleMethod(cls, @selector(_onqueue_didFinishCollectingMetrics:completion:), @selector(_onqueue_didFinishCollectingMetrics:completion:), &error);
	});
}
...
- (void)__dtx_resume
{
	...
	[self ln_attachObject:unique forKey:__DTXConnectionUnique];
...
{
	__DTXProfilerMarkNetworkResponseEnd([self ln_attachedObjectForKey:__DTXConnectionResponse], [self ln_attachedObjectForKey:__DTXConnectionData], arg2, [self ln_attachedObjectForKey:__DTXConnectionUnique], NSDate.date);
